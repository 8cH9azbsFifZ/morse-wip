...
extern "C" void put_rxchar(char *data)
{
	char *cptr = data; 
	while (*cptr != '\0')
...
static integer  elmhat, xhat,imax, retstat; 
static morse *mp = new morse();
PARAMS params = { 
// var 	sym   txtfile txt   xplot  wid  len bfv frq dur  srate delta amp  fft 	agc	spd		dec_ratio
FALSE, FALSE, FALSE, TRUE, FALSE, 8192, 32, 0, 600, 5.0, 8000, 10.0, 0.0, 0, 	0,	20,		20};
...
// normalize if possible
	...
	if (!progStatus.sqlonoff || metric > progStatus.sldrSquelchValue ) {
		if (progdefaults.CWuseSOMdecoding) {
			...
			retstat = mp->proces_(&zout, &rn, &xhat, &px, &elmhat, &spdhat, &imax, &pmax, DEC_RATIO*1000.* (1.0/samplerate));
